...
// initially populate map with all listings
await search();
...
function addListingMarker(map, listing) {
  ...
  markers.addLayer(L.marker([lat, lng]).bindPopup(popupContent)).addTo(map);
}
...
document.querySelector('.clear-button').addEventListener('click', async (e) => {
  clearSearch();
  await search();
})
